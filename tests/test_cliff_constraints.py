...
class TestCliffConstraints:
    ...
    def test_cliff_basic_constraints(self):
        """测试基本的悬崖约束条件"""
        map_gen = TileBasedMap(tile_width=6, tile_height=6)
        ...
        for tile_y in range(map_gen.tile_height):
            for tile_x in range(map_gen.tile_width):
                tile = map_gen.tiles[tile_y][tile_x]
            ...
            for tile_x in range(map_gen.tile_width):
                ...
                if tile and tile.name == "cliff":
                    ...
                    for nx, ny in directions:
                        if 0 <= nx < map_gen.tile_width and 0 <= ny < map_gen.tile_height:
                            neighbor_tile = map_gen.tiles[ny][nx]
                            if neighbor_tile:
                    ...
                    if not (has_plain and has_highland):
                        violations.append(f"悬崖位置({tile_x}, {tile_y}) - 平原:{has_plain}, 高地:{has_highland}")
    ...
    def test_cliff_constraints_multiple_seeds(self, seed):
        ...
        # 如果有悬崖，所有悬崖都必须满足约束
        if cliff_count > 0:
            assert valid_cliffs == cliff_count, f"种子{seed}: {cliff_count}个悬崖中只有{valid_cliffs}个满足约束"
...
class TestMapGeneration:
    ...
    def test_map_generation_with_seed(self):
        """测试使用种子生成地图"""
        map_gen1 = TileBasedMap(tile_width=4, tile_height=4)
        map_gen2 = TileBasedMap(tile_width=4, tile_height=4)
    ...
    def test_valid_templates_selection(self):
        """测试有效模板选择"""
        map_gen = TileBasedMap(tile_width=4, tile_height=4)
        
        # 在空地图的第一个位置，应该有有效的模板选择
        valid_templates = map_gen.get_valid_templates(0, 0)
        assert len(valid_templates) > 0, "应该有至少一个有效模板"
